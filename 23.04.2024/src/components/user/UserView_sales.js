...
function UserView_sales() {
  ...
  const deleteData = async () => {
    ...
    await axios
      .post(
        "https://vebbox.in/gvmbackend/controllers/api/delete/salesDelete.php",
        payload
      )
      .then((res) => {
  ...
  const handleSubmit = (event) => {
    ...
    if (form.checkValidity() === false) {
      ...
      const newData = {
        sales_id: selectedPurchase.sales_id,
        ...
        .then((response) => {
          ...
          setPurchaseDataContext((prevPurchaseData) => {
            const updatedPurchaseData = prevPurchaseData.map((purchase) => {
              if (purchase.sales_id === selectedPurchase.sales_id) {
                return newData;
              } else {
                return purchase;
              }
  ...
  const dateSubmit = () => {
    ...
    axios
      .post(
        "https://vebbox.in/gvmbackend/controllers/api/get/salesFliter.php",
        data
      )
      .then((response) => {
        setFilteredData(response.data);
                ...
                <thead>
                  <tr>
                    ...
                    <th onClick={() => handleSort("customer_name")}>
                      customer_name {getSortArrow("customer_name")}
                    </th>
                    <th>customer_type</th>
                    <th>location</th>
                    <th>site_name</th>
                    ...
                    >
                      <Col xs={12} md={6}>
                        ...
                        <Name
                          ...
                          value={updateData?.customer_name}
                        />
                        <Radio
                          name="Customer Type"
                          name1="customer_type"
                          ...
                          value2="Existing"
                          handleDataChange={handleInputChange}
                          choose={updateData?.customer_type}
                        />
                        <Name
                          name="Location"
                          handleDataChange={handleInputChange}
